...
app.layout = html.Div([
    html.Div([
        html.H1("CCC Clock Live Wall Dashboard", 
                style={'textAlign': 'center', 'color': '#2c3e50', 'marginBottom': '30px'}),
        html.P("Real-time monitoring of CCC Clock demonstration system parameters",
    ...
    ], style={'backgroundColor': '#f8f9fa', 'padding': '15px', 'marginTop': '20px'}),
...
def update_plots(n, time_window_minutes):
    ...
    parity_fig.add_hline(y=0.5, line_dash="dash", line_color="gray", 
    ...
    witness1_fig.add_trace(go.Scatter(
        x=filtered_times,
        y=filtered_lo,
        mode='lines+markers',
        name='LO Amplitude',
    ...
    witness2_fig.add_trace(go.Scatter(
        x=filtered_times,
    ...
    status_color = '#27ae60' if latest_snr > 20 and abs(latest_parity - 0.5) < 0.1 else '#e74c3c'
    status_text = 'NOMINAL' if status_color == '#27ae60' else 'ALERT'
    
    status_div = html.Div([
        html.Span(f"Status: {status_text}", 
        ...
        html.Br(),
        html.Span(f"Last Update: {filtered_times[-1].strftime('%H:%M:%S')}" if filtered_times else "No data")
    ])
