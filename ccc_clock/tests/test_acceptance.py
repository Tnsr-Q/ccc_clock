...
class TestAcceptanceCriteria:
    ...
    def setup_method(self):
        ...
        self.test_loop = ThetaLoop(
            r_star_min=0.9, r_star_max=1.1,
            theta_min=0.0, theta_max=np.pi/8
        )
    ...
    def test_a1_time_to_detect_criterion(self):
        ...
        if successful_cases:
            ...
            for case in successful_cases[:3]:  # Show first 3
                print(f"     {case['parameter_set']}: A_Σ={case['A_Sigma']:.2e}, τ_req={case['tau_req_hours']:.1f}h")
    ...
    def test_a3_sign_flip_criterion(self):
        ...
        all_ortho_tests_passed = all(result['passed'] for result in test_results.values())
        ...
        assert abs(signal_ratio + 1) < 0.5, f"Signal ratio {signal_ratio:.3f} not close to -1"
