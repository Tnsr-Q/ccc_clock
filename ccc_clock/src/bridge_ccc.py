...
try:
    from bridge_null import (
        BridgeEdge, bridge_null_refined, cycle_residual_fro,
        commutator_diagnostics, alpha_hat_from_history,
        local_se, aggregate_edge, make_random_edges
    )
    from experiments_bridge_null import (
        cycle_residual_spec, bridge_null_spectral,
        create_identical_copies, optimize_weights
    )
    from bridge_null_utils import (
        compute_proportionality_metrics, joint_diag,
        apply_joint_diag_to_edges
    )
...
class CCCBridgeAnalyzer:
    ...
    def create_ccc_edges(self, n_dim: int = 4, n_edges: int = 3, 
                        complexity_factor: float = 1.0) -> List[BridgeEdge]:
        """
    ...
    def epsilon_continuation_analysis(self, edges: List[BridgeEdge],
                                    eps_start: float = 1e-2,
                                    eps_target: float = 1e-9,
                                    eps_factor: float = 0.5) -> Dict:
        """
        ...
        alpha_estimates = [r / (eps * max_ND_comm + 1e-15) for r, eps in zip(residuals, eps_values)]
    ...
    def analyze_ccc_bridge(self, edges: List[BridgeEdge],
                          eps_start: float = 1e-2,
                          eps_target: float = 1e-9) -> CCCBridgeResult:
        """
        ...
        if BRIDGE_TOOLS_AVAILABLE:
            se_info = local_se(edges, eps_analysis['R_star'], 
                             eps=eps_analysis['eps_values'][-1])
...
def epsilon_continuation(edges: List[BridgeEdge], eps_values: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
...
if __name__ == "__main__":
    ...
    if validation['A2_met']:
        print("  ✅ A2 criterion met: Bridge analysis complete with R*, SE, α, and ε-sweep")
    else:
