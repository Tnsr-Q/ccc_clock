...
class CCCMetrology:
    ...
    def compute_operational_curvature(self, params: ParameterSet, 
                                    S_e_dot: Optional[float] = None) -> float:
        """
    ...
    def compute_clock_observable(self, params: ParameterSet, A_Sigma: float,
                               systematics: float = 0.0, S_e_dot: Optional[float] = None) -> float:
        """
    ...
    def compute_snr(self, params: ParameterSet, A_Sigma: float, sigma_0: float,
                   tau: float, S_e_dot: Optional[float] = None) -> float:
        """
    ...
    def compute_time_to_detect(self, params: ParameterSet, A_Sigma: float,
                             sigma_0: float, z_alpha: float = 3.0,
                             S_e_dot: Optional[float] = None) -> float:
        """
    ...
    def sensitivity_analysis(self, A_Sigma_range: np.ndarray,
                           sigma_0_values: List[float] = [1e-17, 3e-18],
                           z_alpha: float = 3.0) -> Dict:
        """
        ...
        for param_name, params in PARAMETER_SETS.items():
            ...
            for i, sigma_0 in enumerate(sigma_0_values):
                for j, A_Sigma in enumerate(A_Sigma_range):
                    tau_req = self.compute_time_to_detect(params, A_Sigma, sigma_0, z_alpha)
...
class ABBASimulator:
    ...
    def simulate_lock_in_trace(self, params: ParameterSet, A_Sigma: float,
                             duration: float = 3600, sigma_0: float = 3e-18,
                             loop_reversed: bool = False, 
                             systematics_amplitude: float = 1e-19) -> Dict:
        """
        ...
        systematics += systematics_amplitude * 0.5 * np.sin(2 * np.pi * 1.0 * t)  # Faster oscillation
        ...
        demod_filtered = np.convolve(demod_signal, np.ones(filter_window)/filter_window, mode='same')
        ...
        for tau in tau_values:
            ...
            if window_samples < len(demod_filtered):
                ...
                windowed_std = np.std(demod_filtered[-window_samples:]) / np.sqrt(window_samples)
        ...
        return {
            ...
            'final_demod_value': np.mean(demod_filtered[-int(60/self.dt):]),  # Last minute average
            'theoretical_snr': metrology.compute_snr(params, A_Sigma, sigma_0, duration)
        }
    
    def demonstrate_sign_flip(self, params: ParameterSet, A_Sigma: float,
                            duration: float = 1800) -> Dict:
        """
        ...
        return {
            ...
            'ratio': trace_reversed['final_demod_value'] / trace_normal['final_demod_value']
        }

def compute_operational_curvature(K_dot: float, S_e_dot: float, S_loss_dot: float) -> float:
    """Standalone function to compute operational curvature ratio."""
    return K_dot / (S_e_dot + S_loss_dot)

def compute_snr(Gamma_Theta: float, R_op: float, A_Sigma: float, 
               sigma_0: float, tau: float) -> float:
    """Standalone function to compute SNR."""
    signal = Gamma_Theta * R_op * A_Sigma
    noise = sigma_0 / np.sqrt(tau)
    return signal / noise

def compute_time_to_detect(Gamma_Theta: float, R_op: float, A_Sigma: float,
                          sigma_0: float, z_alpha: float = 3.0) -> float:
...
if __name__ == "__main__":
    ...
    sign_flip_demo = simulator.demonstrate_sign_flip(params_test, A_Sigma=1e-6, duration=300)
