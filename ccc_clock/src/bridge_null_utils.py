...
def compute_proportionality_metrics(N: np.ndarray, D: np.ndarray) -> Tuple[float, float]:
    """
...
def joint_diag(N: np.ndarray, D: np.ndarray, 
    ...
    """
    n = N.shape[0]
    assert N.shape == (n, n) and D.shape == (n, n), "Matrices must be square and same size"
    ...
    for iteration in range(max_iter):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                N_sub = np.array([[N_work[i,i], N_work[i,j]], 
                                 [N_work[j,i], N_work[j,j]]], dtype=np.complex128)
                D_sub = np.array([[D_work[i,i], D_work[i,j]], 
                                 [D_work[j,i], D_work[j,j]]], dtype=np.complex128)
                
                # Current off-diagonal contribution
                current_off = np.abs(N_sub[0,1])**2 + np.abs(N_sub[1,0])**2 + \
                             np.abs(D_sub[0,1])**2 + np.abs(D_sub[1,0])**2
                ...
                for angle in np.linspace(0, np.pi/2, 20):
                    ...
                    new_off = np.abs(N_rot[0,1])**2 + np.abs(N_rot[1,0])**2 + \
                             np.abs(D_rot[0,1])**2 + np.abs(D_rot[1,0])**2
...
def apply_joint_diag_to_edges(edges: List, joint_diag_enabled: bool = False, 
                             tol: float = 1e-8, max_iter: int = 500) -> Tuple[List, Dict]:
    """
...
def run_self_tests():
    ...
    Q = np.array([[1/np.sqrt(2), 1/np.sqrt(2)], [1/np.sqrt(2), -1/np.sqrt(2)]], dtype=np.complex128)
    ...
    off_diag_N = np.sum(np.abs(N3_transformed)**2) - np.sum(np.abs(np.diag(N3_transformed))**2)
    off_diag_D = np.sum(np.abs(D3_transformed)**2) - np.sum(np.abs(np.diag(D3_transformed))**2)
    
    print(f"    ✅ Converged: {jd_info['converged']}, iterations: {jd_info['iterations']}")
